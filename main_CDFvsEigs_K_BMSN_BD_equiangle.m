...
Algorithms = ["BMSN-BF","BMSN-GE","MMSE","GMI1","GMI2","BD"]; %,"BMSN-GE3"
...
for isimu = 1:SIMU % 試行回数のループ
    for n = 1 : Nu
        % 幾何学レイリーチャネル
        % マルチパス波L波が等間隔角度で到来するとしたら
        Thetat = -90:180/(NL-1):90; % randsample(Thetat,1)
        Thetar = -90:180/(NL-1):90; % randsample(Thetar,1)
        for L = 1:NL
            ThetatL = randsample(Thetat,1);
            ThetarL = randsample(Thetar,1);
            gtheta_t = An * ((cos(ThetatL*derad))^(cos_n/2));
            gtheta_r = An * ((cos(ThetarL*derad))^(cos_n/2));
            a_t_iid_cos(:,:,L) = gtheta_t * sqrt(1/Nt) * exp(1j*ThetatL*derad) * exp(-1j*2*pi*d_t*(0:Nt-1).' * sin(ThetatL*derad)); % ユーザ毎のNLoS送信モードベクトル An*cos(Theta_t(1,n)*derad) *
            a_r_iid_cos(:,:,L) = gtheta_r * sqrt(1/Nr) * exp(-1j*2*pi*d_r*(0:Nr-1).' * sin(ThetarL*derad)); % ユーザ毎のNLoS受信モードベクトル An*cos(Theta_r(1,n)*derad) *
            X_cos(:,:,L) = (a_r_iid_cos(:,:,L) * a_t_iid_cos(:,:,L)');
            a_t_iid_iso(:,:,L) = sqrt(1/Nt) * exp(1j*ThetatL*derad) * exp(-1j*2*pi*d_t*(0:Nt-1).' * sin(ThetatL*derad)); % ユーザ毎のNLoS送信モードベクトル An*cos(Theta_t(1,n)*derad) *
            a_r_iid_iso(:,:,L) = sqrt(1/Nr) * exp(-1j*2*pi*d_r*(0:Nr-1).' * sin(ThetarL*derad)); % ユーザ毎のNLoS受信モードベクトル An*cos(Theta_r(1,n)*derad) *
...
%     plot(rr3(:,9),Y,'k-','Linewidth',2); plot(rr3(:,10),Y,'k--','Linewidth',2);
    plot(rr3_iso(:,11),Y,'k-','Linewidth',2); plot(rr3_iso(:,12),Y,'k--','Linewidth',2);
...
%     plot(rr3_cos(:,9),Y,'k-o','MarkerIndices',10:100:length(Y),'Linewidth',2); plot(rr3_cos(:,10),Y,'k--o','MarkerIndices',50:100:length(Y),'Linewidth',2);
    plot(rr3_cos(:,11),Y,'k-o','MarkerIndices',10:100:length(Y),'Linewidth',2); plot(rr3_cos(:,12),Y,'k--o','MarkerIndices',50:100:length(Y),'Linewidth',2);
