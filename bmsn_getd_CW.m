...
function [W,UTT,STT,RIP,SP] = bmsn_getd_CW(NT,NR,NU,H,a)
Wopt=zeros(NT,NR,NU);W=zeros(NT,NR-1,NU);
...
for nuser=1:NU
    ...
    %%
    Wopt(:,:,nuser) = EW(:,1:NR)*(log2(D2(:,:,nuser)+1)); % Weighting by sqrt of SINR
    ...
    if NR > 1
        W(:,:,nuser)=Wopt(:,:,nuser)*VTT(:,1:NR-1,nuser); %lambda1に対応する固有ベクトルのみをふたつ使用
    else
...
for nuser=1:NU
    ...
    if NR > 1
        YS=UTT(:,1:NR-1,nuser)'*H(ns,:)*W(:,:,nuser);
    else
