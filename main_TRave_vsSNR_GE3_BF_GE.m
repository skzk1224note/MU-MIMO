...
%         [W_ZF,U_ZF,S_ZF,RIPZF] = zf(NT,NR,NU,H0); % function, zf.m を使用
        ...
        a=NT/(10^(SN_tar/10));
        [W_BMSNBF,U_BMSNBF,S_BMSNBF,RIPBF] = bmsn_bf(NT,NR,NU,H0,a,T); % function, bmsn_bf.m を使用
    
        % BMSN-GE algorithm with adapted pseudo-noise power
        a=NT/(10^(SN_tar/10));
        [W_BMSNGE,U_BMSNGE,S_BMSNGE,RIPGE] = bmsn_gev(NT,NR,NU,H0,a); % function, bmsn_gev.m を使用
        
        % BMSN-GE-TD algorithm with adapted pseudo-noise power
        a=NT/(10^(SN_tar/10));
        [W_BMSNGEc,U_BMSNGEc,S_BMSNGEc,RIPGEc] = bmsn_getd_3(NT,NR,NU,H0,a); % function, bmsn_ge3.m を使用
        
        % BD-AS algorithm
        %[W_BDAS,U_BDAS,S_BDAS,RIPBDAS] = bd_as(NT,NR,NRs,NU,H0); % function, bd_as.m を使用
...
%     Qm(:,5) = mean(sort(Q(:,5,nuser),1),3);
  %  Qm(:,5) = mean(sort(Q(:,5,nuser),1),3);
    %GEとTDでそれぞれのSNRのTRが1000回のうち何回上回っているか
    if Qm(:,2)<=Qm(:,3)
       e(:,1)=100;
    else
       e(:,1)=0;
...
%plot(SNR,QmC(:,1),'-',SNR,QmC(:,2),'-',SNR,QmC(:,3),'-','Linewidth',2);
%plot(SNR,QmC,'Linewidth',2);
plot(SNR,em,'-');
set(gca,'Fontsize',14,'Fontname','Times New Roman');
%legend('BMSN-BF','BMSN-GE','BMSN-GE-ATD2','Location','Northwest');
xlabel('SNR [dB]','Fontsize',16,'Fontname','Times New Roman');
%ylabel('change CDF','Fontsize',16,'Fontname','Times New Roman')
