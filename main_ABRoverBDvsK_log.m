...
% plot(K_box,QmC_overBD(:,1),'r-d',K_box,QmC_overBD(:,2),'b-s',K_box,QmC_overBD(:,3),'g-x',...
%     K_box,QmC_overBD(:,5),'m-^','Linewidth',2);

%%%%%%%%%%%%% 縦軸対数表現
semilogy(K_box,QmC_overBD(:,1),'r-d',K_box,QmC_overBD(:,2),'b-s',K_box,QmC_overBD(:,3),'g-x',K_box,QmC_overBD(:,5),'m-^','Linewidth',2);

% axis([K_min,K_max,0,10])

axis([K_min,K_max,1e-1,1e1])           % 縦軸対数表現
set(gca,'Fontsize',14,'Fontname','Arial','YTick',[0.1,1,10]);   % 縦軸対数表現
set(gca,'Fontsize',14,'Fontname','Arial');
legend('BMSN-BF','BMSN-GE','MMSE-CI','ZF-CI','Location','Northwest');
xlabel('K [dB]','Fontsize',16,'Fontname','Arial');
ylabel('ABR/ABR_B_D','Fontsize',16,'Fontname','Arial');
