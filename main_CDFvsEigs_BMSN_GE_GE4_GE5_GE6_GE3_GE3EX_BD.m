...
Algorithms = ["BMSN-GE","BMSN-GE4","BMSN-GE5","BMSN-GE6","BMSN-GE3",'BMSN-GE3EX',"BD"]; %,"BMSN-GE3","MMSE","GMI1","GMI2","ZF"
...
for isimu = 1:SIMU % 試行回数のループ
    ...
    for Directivity_switch = 0:2 % 送受信素子の指向性考慮の有無 0:無,1:有
        ...
        % BMSN-GE4 algorithm
        [W_BMSN_GE4,U_BMSN_GE4,S_BMSN_GE4,RIPGE4,~] = bmsn_ge4(Nt,Nr,Nu,H0,a); % function bmsn_ge4.m を使用
        
        % BMSN-GE5 algorithm
        [W_BMSN_GE5,U_BMSN_GE5,S_BMSN_GE5,RIPGE5,~] = bmsn_ge5(Nt,Nr,Nu,H0,a); % function bmsn_ge5 を使用
        
        % BMSN-GE6 algorithm
        [W_BMSN_GE6,U_BMSN_GE6,S_BMSN_GE6,RIPGE6,~] = bmsn_ge6(Nt,Nr,Nu,H0,a); % function bmsn_ge6 を使用
        
        % BMSN-GE3 algorithm
        [W_BMSN_GE3,U_BMSN_GE3,S_BMSN_GE3,RIPGE3,~] = bmsn_ge3(Nt,Nr,Nu,H0,a); % function bmsn_ge3 を使用 
        
        % BMSN-GE3EX algorithm
        [W_BMSN_GE3EX,U_BMSN_GE3EX,S_BMSN_GE3EX,RIPGE3EX,~] = bmsn_ge3EX(Nt,Nr,Nu,H0,a); % function bmsn_ge3EX を使用 
        ...
        if Directivity_switch == 1
            for nuser=1:Nu
                if Nr==1
                    ...
                    E_cos4(isimu,:,nuser,strcmp("BMSN-GE4",Algorithms)) = 10*log10((S_BMSN_GE4(1,1,nuser).^2)./(RIPGE4(1,nuser)+Nt*snt));
                    E_cos4(isimu,:,nuser,strcmp("BMSN-GE5",Algorithms)) = 10*log10((S_BMSN_GE5(1,1,nuser).^2)./(RIPGE5(1,nuser)+Nt*snt));
                    E_cos4(isimu,:,nuser,strcmp("BMSN-GE6",Algorithms)) = 10*log10((S_BMSN_GE6(1,1,nuser).^2)./(RIPGE6(1,nuser)+Nt*snt));
                    E_cos4(isimu,:,nuser,strcmp("BMSN-GE3",Algorithms)) = 10*log10((S_BMSN_GE3(1,1,nuser).^2)./(RIPGE3(1,nuser)+Nt*snt));
                    E_cos4(isimu,:,nuser,strcmp("BMSN-GE3EX",Algorithms)) = 10*log10((S_BMSN_GE3EX(1,1,nuser).^2)./(RIPGE3EX(1,nuser)+Nt*snt));
...
%                     E_cos4(isimu,:,nuser,strcmp("ZF",Algorithms)) = 10*log10(S_ZF(1,1,nuser).^2/(Nt*snt));    
                    ...
                    E_cos4(isimu,:,nuser,strcmp("BMSN-GE4",Algorithms)) = 10*log10((diag(S_BMSN_GE4(:,:,nuser)).^2)./(RIPGE4(:,nuser)+Nt*snt));
                    E_cos4(isimu,:,nuser,strcmp("BMSN-GE5",Algorithms)) = 10*log10((diag(S_BMSN_GE5(:,:,nuser)).^2)./(RIPGE5(:,nuser)+Nt*snt));
                    E_cos4(isimu,:,nuser,strcmp("BMSN-GE6",Algorithms)) = 10*log10((diag(S_BMSN_GE6(:,:,nuser)).^2)./(RIPGE6(:,nuser)+Nt*snt));
                    E_cos4(isimu,:,nuser,strcmp("BMSN-GE3",Algorithms)) = 10*log10((diag(S_BMSN_GE3(:,:,nuser)).^2)./(RIPGE3(:,nuser)+Nt*snt));
                    E_cos4(isimu,:,nuser,strcmp("BMSN-GE3EX",Algorithms)) = 10*log10((diag(S_BMSN_GE3EX(:,:,nuser)).^2)./(RIPGE3EX(:,nuser)+Nt*snt));
        ...
        elseif Directivity_switch == 0
            for nuser=1:Nu
                if Nr==1
                    ...
                    E_cos0(isimu,:,nuser,strcmp("BMSN-GE4",Algorithms)) = 10*log10((S_BMSN_GE4(1,1,nuser).^2)./(RIPGE4(1,nuser)+Nt*snt));
                    E_cos0(isimu,:,nuser,strcmp("BMSN-GE5",Algorithms)) = 10*log10((S_BMSN_GE5(1,1,nuser).^2)./(RIPGE5(1,nuser)+Nt*snt));
                    E_cos0(isimu,:,nuser,strcmp("BMSN-GE6",Algorithms)) = 10*log10((S_BMSN_GE6(1,1,nuser).^2)./(RIPGE6(1,nuser)+Nt*snt));
                    E_cos0(isimu,:,nuser,strcmp("BMSN-GE3",Algorithms)) = 10*log10((S_BMSN_GE3(1,1,nuser).^2)./(RIPGE3(1,nuser)+Nt*snt));
                    E_cos0(isimu,:,nuser,strcmp("BMSN-GE3EX",Algorithms)) = 10*log10((S_BMSN_GE3EX(1,1,nuser).^2)./(RIPGE3EX(1,nuser)+Nt*snt));
...
%                     E_iso(isimu,:,nuser,strcmp("ZF",Algorithms)) = 10*log10(S_ZF(1,1,nuser).^2/(Nt*snt));    
                    ...
                    E_cos0(isimu,:,nuser,strcmp("BMSN-GE4",Algorithms)) = 10*log10((diag(S_BMSN_GE4(:,:,nuser)).^2)./(RIPGE4(:,nuser)+Nt*snt));
                    E_cos0(isimu,:,nuser,strcmp("BMSN-GE5",Algorithms)) = 10*log10((diag(S_BMSN_GE5(:,:,nuser)).^2)./(RIPGE5(:,nuser)+Nt*snt));
                    E_cos0(isimu,:,nuser,strcmp("BMSN-GE6",Algorithms)) = 10*log10((diag(S_BMSN_GE6(:,:,nuser)).^2)./(RIPGE6(:,nuser)+Nt*snt));
                    E_cos0(isimu,:,nuser,strcmp("BMSN-GE3",Algorithms)) = 10*log10((diag(S_BMSN_GE3(:,:,nuser)).^2)./(RIPGE3(:,nuser)+Nt*snt));
                    E_cos0(isimu,:,nuser,strcmp("BMSN-GE3EX",Algorithms)) = 10*log10((diag(S_BMSN_GE3EX(:,:,nuser)).^2)./(RIPGE3EX(:,nuser)+Nt*snt));
...
%                     E_iso(isimu,:,nuser,strcmp("ZF",Algorithms)) = 10*log10(diag(S_ZF(:,:,nuser)).^2/(Nt*snt));
            ...
            for nuser=1:Nu
                if Nr==1
                    ...
                    E(isimu,:,nuser,strcmp("BMSN-GE4",Algorithms)) = 10*log10((S_BMSN_GE4(1,1,nuser).^2)./(RIPGE4(1,nuser)+Nt*snt));
                    E(isimu,:,nuser,strcmp("BMSN_GE5",Algorithms)) = 10*log10((S_BMSN_GE5(1,1,nuser).^2)./(RIPGE5(1,nuser)+Nt*snt));
                    E(isimu,:,nuser,strcmp("BMSN_GE6",Algorithms)) = 10*log10((S_BMSN_GE6(1,1,nuser).^2)./(RIPGE6(1,nuser)+Nt*snt));
                    E(isimu,:,nuser,strcmp("BMSN_GE3",Algorithms)) = 10*log10((S_BMSN_GE3(1,1,nuser).^2)./(RIPGE3(1,nuser)+Nt*snt));
                     E(isimu,:,nuser,strcmp("BMSN_GE3EX",Algorithms)) = 10*log10((S_BMSN_GE3EX(1,1,nuser).^2)./(RIPGE3EX(1,nuser)+Nt*snt));
...
%                     E(isimu,:,nuser,strcmp("ZF",Algorithms)) = 10*log10(S_ZF(1,1,nuser).^2/(Nt*snt));    
                    ...
                    E(isimu,:,nuser,strcmp("BMSN-GE4",Algorithms)) = 10*log10((diag(S_BMSN_GE4(:,:,nuser)).^2)./(RIPGE4(:,nuser)+Nt*snt));
                    E(isimu,:,nuser,strcmp("BMSN-GE5",Algorithms)) = 10*log10((diag(S_BMSN_GE5(:,:,nuser)).^2)./(RIPGE5(:,nuser)+Nt*snt));
                    E(isimu,:,nuser,strcmp("BMSN-GE6",Algorithms)) = 10*log10((diag(S_BMSN_GE6(:,:,nuser)).^2)./(RIPGE6(:,nuser)+Nt*snt));
                    E(isimu,:,nuser,strcmp("BMSN-GE3",Algorithms)) = 10*log10((diag(S_BMSN_GE3(:,:,nuser)).^2)./(RIPGE3(:,nuser)+Nt*snt));
                    E(isimu,:,nuser,strcmp("BMSN-GE3EX",Algorithms)) = 10*log10((diag(S_BMSN_GE3EX(:,:,nuser)).^2)./(RIPGE3EX(:,nuser)+Nt*snt));
...
Holizon_min = round(min(min(rr3(:,8),min(rr3(:,14)))))-5;
...
if Nr==2
    plot(rr3(:,1),Y,'Color',[1 0 0],'LineStyle','-','MarkerIndices',10:100:length(Y),'Linewidth',2); plot(rr3(:,2),Y,'Color',[1 0 0],'LineStyle','--','MarkerIndices',50:100:length(Y),'Linewidth',2);
    plot(rr3(:,3),Y,'Color',[0 0 1],'LineStyle','-','MarkerIndices',10:100:length(Y),'Linewidth',2); plot(rr3(:,4),Y,'Color',[0 0 1],'LineStyle','--','MarkerIndices',50:100:length(Y),'Linewidth',2);
    plot(rr3(:,5),Y,'Color',[0 1 0],'LineStyle','-','MarkerIndices',10:100:length(Y),'Linewidth',2); plot(rr3(:,6),Y,'Color',[0 1 0],'LineStyle','--','MarkerIndices',50:100:length(Y),'Linewidth',2);
    plot(rr3(:,7),Y,'Color',[1 0 1],'LineStyle','-','MarkerIndices',10:100:length(Y),'Linewidth',2); plot(rr3(:,8),Y,'Color',[1 0 1],'LineStyle','--','MarkerIndices',50:100:length(Y),'Linewidth',2);
    plot(rr3(:,9),Y,'Color',[1 0.65 0],'LineStyle','-','MarkerIndices',10:100:length(Y),'Linewidth',2); %plot(rr3(:,10),Y,'Color',[1 0.65 0],'LineStyle','--','MarkerIndices',10:100:length(Y),'Linewidth',2); 
    plot(rr3(:,11),Y,'Color',[0 0.5 0.5],'LineStyle','-','MarkerIndices',10:100:length(Y),'Linewidth',2); plot(rr3(:,12),Y,'Color',[0 0.5 0.5],'LineStyle','--','MarkerIndices',50:100:length(Y),'Linewidth',2);
    plot(rr3(:,13),Y,'Color',[0.5 0 0.5],'LineStyle','-','MarkerIndices',10:100:length(Y),'Linewidth',2); plot(rr3(:,14),Y,'Color',[0.5 0 0.5],'LineStyle','--','MarkerIndices',50:100:length(Y),'Linewidth',2);
...
%       plot(rr3(:,3),Y,'b-^','MarkerIndices',10:100:length(Y),'Linewidth',2); plot(rr3(:,4),Y,'b--^','MarkerIndices',50:100:length(Y),'Linewidth',2);
    ...
    lgd.NumColumns = 1; % 凡例の列数を指定
     legend('BMSN-GE \lambda_1','BMSN-GE \lambda_2',...
        'BMSN-GE (log(1+\lambda)) \lambda_1','BMSN-GE (log(1+\lambda)) \lambda_2',...
        'BMSN-GE (√\lambda) \lambda_1','BMSN-GE (√\lambda) \lambda_2',...
        'BMSN-GE (log(1+\lambda))^2 \lambda_1','BMSN-GE (log(1+\lambda))^2 \lambda_2',...
        'BMSN-GEcopy \lambda_1',... %'BMSN-GEcopy \lambda_2',
        'BMSN-GEcopy (\lambda_2<0.1) \lambda_1','BMSN-GEcopy (\lambda_2<0.1) \lambda_2',...
