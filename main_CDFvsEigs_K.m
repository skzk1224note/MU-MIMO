...
for Directivity_switch = 0:1 % 送受信素子の指向性考慮の有無 0:無,1:有
    for isimu = 1:SIMU % 試行回数のループ
        if Directivity_switch == 1 % 送受信素子の指向性考慮有り
            ...
            % LoS チャネル
            for n = 1 : Nu
                a_t = exp(1j*Theta_t(1,n)*derad) * exp(-1j*2*pi*d_t*(0:Nt-1).'*sin(Theta_t(1,n)*derad)); % ユーザ毎の送信モードベクトル An*cos(Theta_t(1,n)*derad) *
                ...
                for L = 1:NL
                    ...
                    a_t_iid(:,:,L) = gtheta_t * sqrt(1/Nt) * exp(1j*Thetat*derad) * exp(-1j*2*pi*d_t*(0:Nt-1).' * sin(Thetat*derad)); % ユーザ毎のNLoS送信モードベクトル An*cos(Theta_t(1,n)*derad) *
                    a_r_iid(:,:,L) = gtheta_r * sqrt(1/Nr) * exp(-1j*2*pi*d_r*(0:Nr-1).' * sin(Thetar*derad)); % ユーザ毎のNLoS受信モードベクトル An*cos(Theta_r(1,n)*derad) *
...
%                 a_r_iid = a_r_iid.* g_theta_r_iid';
        ...
        elseif Directivity_switch == 0 % 送受信素子の指向性考慮無し
            ...
            for n = 1 : Nu
                a_t = sqrt(1/Nt) * exp(1j*Theta_t(1,n)*derad) * exp(-1j*2*pi*d_t*(0:Nt-1).'*sin(Theta_t(1,n)*derad)); % ユーザ毎の送信モードベクトル
                a_r = sqrt(1/Nr) * exp(-1j*2*pi*d_r*(0:Nr-1).'*sin(Theta_r(1,n)*derad)); % ユーザ毎の受信モードベクトル
...
%         H_iid = (randn(Nr*Nu,Nt)+1j*randn(Nr*Nu,Nt))/sqrt(2);
        ...
        % BMSN-GE algorithm
        [W_BMSN_GE,U_BMSN_GE,S_BMSN_GE,RIPGE,~] = bmsn_ge4(Nt,Nr,Nu,H0,a); % function bmsn_ge4.m を使用
