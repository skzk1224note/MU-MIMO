...
%     Wopt_k2(:,:,inu) = Wopt_k2(:,:,inu)/norm(Wopt_k2(:,:,inu),'fro')*sqrt(Nr);
    ...
    D12=D1(:,1)/D1(:,2);
    SNR1(inu,isnr)=D1(:,1)./a;
    SNR2(inu,isnr)=D1(:,2)./a;
    EW = EW(:,IN);
    %固有値のルートの値をD3として格納
    D3=sqrt(D1);
...
%     Wopt_k2(:,:,inu) = Wopt_k2(:,:,inu)/norm(Wopt_k2(:,:,inu),'fro')*sqrt(Nr);
    ...
    %% BMSN2-GE-ATD1 α=sigma2 NT equal to MMSE  
      A = HeHe+a*eye(size(He(:,:,inu),2));
    B = Hu(:,:,inu)'*Hu(:,:,inu);
    [EW,D] = eig(B,A);
    [D1,IN] = sort(diag(abs(D)).','descend');
    %D12(inu,isnr)=D1(:,1)/D1(:,2);
...
%     Wopt_k2(:,:,inu) = Wopt_k2(:,:,inu)/norm(Wopt_k2(:,:,inu),'fro')*sqrt(Nr);
    ...
    %Wr(ユーザ側ウエイト)
    [Ut,Std,Vt] = svd(Hu(:,:,inu)*Wopt);
    St_BMSN2geatd1(:,inu) = diag(Std(1:Nr,1:Nr));
    Wr_BMSN2geatd1(:,:,inu) = Ut';
    Vts_BMSN2geatd1(:,:,inu) = Vt(:,1:Nr);
    Wtu_BMSN2geatd1(:,:,inu) = Wopt*Vts_BMSN2geatd1(:,:,inu);
...
for inu = 1:Nu
    ...
    for ii = 1:size(pattern,1)
        ...
        Hs_BMSN2gc = zeros(Nr,ndata);
        Hs_BMSN2geatd1 = zeros(Nr,ndata);% ユーザiの受信信号（受信重み付け前）
        for ij = 1:Nu
            ...
            Hs_BMSN2geatd1= Hs_BMSN2geatd1 + Hu(:,:,inu)*Wtu_BMSN2geatd1(:,:,ij)*s(:,:,ii,ij)*sqrt(amp2);
        end
        ...
        yu_BMSN2geatd1(:,:,ii,inu) = Wr_BMSN2geatd1(:,:,inu)*(Hs_BMSN2gc+n);
        ...
        yu_BMSN2geatd1(1,:,ii,inu) = yu_BMSN2geatd1(1,:,ii,inu)/St_BMSN2geatd1(1,inu)/sqrt(amp2);
        yu_BMSN2geatd1(2,:,ii,inu) = yu_BMSN2geatd1(2,:,ii,inu)/St_BMSN2geatd1(2,inu)/sqrt(amp2);
        Yu_BMSN2geatd1(1,:,ii,inu) = Decode(yu_BMSN2geatd1(1,:,ii,inu),pattern(ii,1));
        Yu_BMSN2geatd1(2,:,ii,inu) = Decode(yu_BMSN2geatd1(2,:,ii,inu),pattern(ii,2));
    end
end
...
if ii == 1
    ...
    ber_BMSN2geatd1(ii,inu) = BER2(BER1(Yu_BMSN2geatd1(1,:,ii,inu),data(1,:,ii,inu),pattern(ii,1)),pattern(1,1));
else
    ...
    ber_BMSN2geatd1(ii,inu) = BER2(BER1(Yu_BMSN2gc(1,:,ii,inu),data(1,:,ii,inu),pattern(ii,1)),pattern(ii,1)) ...
        + BER2(BER1(Yu_BMSN2geatd1(2,:,ii,inu),data(2,:,ii,inu),pattern(ii,2)),pattern(ii,2));
end
end
end
...
ber_min_BMSN2geatd1(isimu,isnr) = mean(min(ber_BMSN2geatd1));
...
BER_BMSN2geatd1 = mean(ber_min_BMSN2geatd1);
BER_BD2 = mean(ber_BD2);

csvwrite(evfile1,[snr;BER_BD]);
csvwrite(evfile2,[snr;BER_BMSN2]);
csvwrite(evfile3,[snr;BER_BMSN2g]);
csvwrite(evfile4,[snr;BER_BMSN2gc]);
csvwrite(evfile4,[snr;BER_BMSN2geatd1]);
csvwrite(evfile5,[snr;BER_BD2]);

toc;

%描写
figure;
semilogy(snr,BER_BMSN2,snr,BER_BMSN2g,snr,BER_BMSN2gc,snr,BER_BMSN2geatd1,snr,BER_BD,snr,BER_BD2,'Linewidth',2);
...
legend('BMSN-BF','BMSN-GE','BMSN-GE-ATD2','BMSN-GE-ATD1','BD','BD-AS','Location','southwest')
