...
MSt_BMSN3=zeros(Nr,LK);
MSt_MMSE=zeros(Nr,LK);
MSt_BMSN1b=zeros(Nr,LK);
...
ber_min_BMSN3(isimu,ik) = mean(min(ber_BMSN3));
ber_min_MMSE(isimu,ik) = mean(min(ber_MMSE));
ber_min_BMSN1b(isimu,ik) = mean(min(ber_BMSN1b));
...
MSt_BMSN3(:,ik)=MSt_BMSN3(:,ik)+mean(St_BMSN3(:,inu),2);
MSt_MMSE(:,ik)=MSt_MMSE(:,ik)+mean(St_MMSE(:,inu),2);
MSt_BMSN1b(:,ik)=MSt_BMSN1b(:,ik)+mean(St_BMSN1b(:,inu),2);
...
MSt_BMSN3(:,ik)=10*log10((MSt_BMSN3(:,ik)/SIMU).^2/(Nt*sigma2));
MSt_MMSE(:,ik)=10*log10((MSt_MMSE(:,ik)/SIMU).^2/(Nt*sigma2));
MSt_BMSN1b(:,ik)=10*log10((MSt_BMSN1b(:,ik)/SIMU).^2/(Nt*sigma2));
fprintf('K = %d dB\n',K_box(ik));
end % ik
...
semilogy(K_box,BER_BD,'-',K_box,BER_BMSN3,K_box,BER_MMSE,'-',K_box,BER_BMSN1b,'-','Linewidth',2);
