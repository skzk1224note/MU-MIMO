...
for isnr = 1:Lsnr
    ...
    for isimu = 1:SIMU
        ...
        for inu = 1:Nu
            ...
            % BMSN-GE3 :一般化固有値問題を解く方法（第2固有値が小さい場合は第1固有値を代入）
            B3(:,:,inu) = He(:,:,inu)'*He(:,:,inu)+a3*eye(size(He(:,:,inu),2));
            A3(:,:,inu) = Hu(:,:,inu)'*Hu(:,:,inu);
            [Wopt_k(:,:,inu),D] = eig(B3(:,:,inu),A3(:,:,inu));
            [D1,IN] = sort(diag(D));
            D2(:,:,inu) = D1.';
            Wopt_k(:,:,inu) = Wopt_k(:,IN,inu);
            Wopt_k2(:,:,inu)=Wopt_k(:,1:Nr,inu);
