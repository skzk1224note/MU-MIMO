...
dt_min = 0.5;
dt_max = 1.0;
dt_box = (dt_min:0.05:dt_max); % 送信アンテナ間隔（in wavelength)
Ldt=length(dt_box);        % dt_boxの大きさ
    ...
    for idt = 1:Ldt 
      ...
      % evfile1 = strcat(folder,fn1,num2str(snr(isnr),'%02d'),'dB_1000itr.csv'); 
        for isimu = 1:SIMU      
          ...
          % LOS チャネル      
            if Directivity_switch == 1 % 送受信素子の指向性考慮有り
             ...
             for n = 1 : Nu
                a_t = exp(-1j*2*pi*dt_tar*(0:Nt-1).'*sin(Theta_t(1,n)*derad))*An*cos(Theta_t(1,n)*derad); % ユーザ毎の送信モードベクトル
            ...
            elseif Directivity_switch == 0 % 送受信素子の指向性考慮無し
             ...
             for n = 1 : Nu
             a_t = exp(-1j*2*pi*dt_tar*(0:Nt-1).'*sin(Theta_t(1,n)*derad)); % ユーザ毎の送信モードベクトル
        ...
        if Directivity_switch == 0 % 指向性無しの場合の行列
            QmC(idt,:)=Qm(round(CDF*SIMU/100),:);% Channel capacity
        elseif Directivity_switch == 1 % 指向性有りの場合の行列
            QmC_cos(idt,:)=Qm(round(CDF*SIMU/100),:);% Channel capacity
        end
        fprintf('dt = %.3g lambda \n',dt_box(idt));
    end % idt end
    %csvwrite(cdffile1,[dt,QmC]);
end
%% グラフ表示 figure ABRvsK
figure;
...
plot(dt_box,QmC(:,1),'r--',dt_box,QmC(:,2),'b--',dt_box,QmC(:,3),'g--',dt_box,QmC(:,4),'y--',dt_box,QmC(:,5),'k--',...
    dt_box,QmC_cos(:,1),'r-',dt_box,QmC_cos(:,2),'b-',dt_box,QmC_cos(:,3),'g-',dt_box,QmC_cos(:,4),'y-',dt_box,QmC_cos(:,5),'k-','Linewidth',2);
axis([dt_min,dt_max,min(min(QmC_cos))-0.05,max(max(QmC_cos))+2])
...
xlabel('{\it{d_t}} [\lambda]','Fontsize',6,'Fontname','Times New Roman');
ylabel('Channel capacity [bits/s/Hz]','Fontsize',6,'Fontname','Times New Roman');
set(gca,'Fontsize',18,'Fontname','Times New Roman');
title(strcat(target_CDF,',',target_SNR,',',target_K,',',target_d_r));
grid on;
hold on;
% End